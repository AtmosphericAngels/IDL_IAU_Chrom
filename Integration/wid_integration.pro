...
;      ID = WIDGET_BUTTON(con_ID, value='Update defaults', uname='update_msinfo', /SEPARATOR)
...
; define general settings  
  ...
  nointtriggers = ['Bad Peak','No Peak Found','Not Integrated', 'ovwr_man']
...
;  IF refresh THEN BEGIN
;    IF TOTAL(STRMATCH(nointtriggers, chrom[sel_chrom].subst[sel_name].ires.comment)) EQ 1 $
...
; New default substances must contain substances in chrom[*].subst, but can contain more
  IF event.id EQ WIDGET_INFO(event.top, find_by_uname='update_msinfo') THEN $
;    upd_defintsettings
    msg=DIALOG_MESSAGE('Feature not avialable yet.', /INFORMATION)
