...
;
; INFO:
; calls specified integration methods. refers to wid_integration
;
; MODIFICATIONS:
...
; - added upsampling option to gauss fit for poorly sampled chromatograms, modified structure
;
;------------------------------------------------------------------------------------------------------------------------
@peak_detection
@int_baseline_gau
      ...
      'gau': $
          BEGIN
            strct=int_gau(x,v, NSIGMA_FIT=sigma, NSIGMA_INT=nsigma_int, NTERMS_BASE=nterms_base, RT_WIN=rt_win, $
                               ...
                               CHK_NOISE=chk_noise, TIMESCALE=timescale, upsample=gau_upsample)
            ...
            te = strct.te
            width = strct.wdth
            upscale = strct.upscale     ; contains upsampling factor GE 1
            t_int_axis = *(strct.t_int_axis) ; pointer            
            flag = strct.flag
            ...
            IF plot THEN BEGIN
              IF STRUPCASE(strct.comment) EQ 'INTEGRATED' THEN BEGIN
               ...
               IF upscale GT 1 THEN BEGIN   ; upsampling was done
                 plot_routine_pobj1, x, v, X_1A=x[fit_pwin], V_1A=peak_fit+base_fit, X_1B=x[fit_pwin], V_1B=base_fit, $
                    X_1E=t_int_axis, V_1E=peak_int+base_int, X_1F=t_int_axis, V_1F=base_int, $
