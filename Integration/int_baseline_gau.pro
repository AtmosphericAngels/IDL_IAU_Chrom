...
FUNCTION int_baseline_gau , xval, yval $
  ...
  IF NOT keyword_set(NTERMS_BASE) THEN nterms_base=2 ; quadratic baseline by default
...
;  nterms = nterms_base+3
  ...
  IF N_ELEMENTS(x) LE nterms_base THEN BEGIN ;if theere is (almost) no data passed, e.g. mass not found in chromatogram
    strct.flag=-1;
